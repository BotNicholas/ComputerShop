...
namespace Computer_magazine
{
    public partial class ComputerEditForm : Form
    {
        ...
        public ComputerEditForm(Form caller, string mode)
        {
            InitializeComponent();
            ...
            {
                connection.Open();
                SqlCommand command = new SqlCommand("select cod_prod from Product where Cod_type = 2000 or Cod_type = 2010", connection); //2000 - PC-es id; 2010 - Laptops id
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (mode == "edit")
            {
                ...
                if(dataGridView1.Rows[e.RowIndex].Cells[4].Value.ToString().ToLower() == "true")
                {
            ...
            else if (mode == "delete")
            {
                if (MessageBox.Show("Are you sure?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string query = $"delete from Computer where cod_prod = {dataGridView1.Rows[e.RowIndex].Cells[0].Value}";
            ...
            if (flag)
            {
                try
                {
                    ...
                    using (connection)
                    {
                        ...
                        if (mode == "add")
                        {
                            query = $"insert into Computer values(@cod_prod, @CPU_freq, @RAM, @HDD, @CD, @Applic)";
                            ...
                            command.Parameters.AddWithValue("@HDD", textBox3.Text);
                            command.Parameters.AddWithValue("@CD", checkBox2.Checked.ToString().ToLowerInvariant());
                        ...
                        else if (mode == "edit")
                        {
                            ...
                            query = $"update Computer set cod_prod = @cod_prod, CPU_freq = @CPU_freq, RAM = @RAM, HDD = @HDD, CD = @CD, Applic = @Applic where cod_prod = {old_id}";
